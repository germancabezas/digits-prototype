<template>
  <div class="w-64 h-full flex flex-col border-r border-gray-200 bg-gradient-to-b from-white via-fuchsia-50 to-yellow-50">
    <!-- Logo/Brand -->
    ...
    <nav class="flex-1 p-4">
      <ul class="space-y-2">
        <li>
          <a
            href="#"
            class="flex items-center px-3 py-2 text-sm font-medium text-gray-700 rounded-md  hover:text-gray-900"
            :class="{ 'bg-emerald-100 text-gray-900': currentView === 'dashboard' }"
        ...
        <li>
          <a
            href="#"
            class="flex items-center px-3 py-2 text-sm font-medium text-gray-700 rounded-md  hover:text-gray-900"
            :class="{ 'bg-emerald-100 text-gray-900': currentView === 'expenses' }"
        ...
        <li>
          <a
            href="#"
            class="flex items-center px-3 py-2 text-sm font-medium text-gray-700 rounded-md  hover:text-gray-900"
            :class="{ 'bg-emerald-100 text-gray-900': currentView === 'reports' }"
        ...
        <li>
          <a
            href="#"
            class="flex items-center px-3 py-2 text-sm font-medium text-gray-700 rounded-md  hover:text-gray-900"
            :class="{ 'bg-emerald-100 text-gray-900': currentView === 'messages' }"
    ...
    <div class="p-4 border-t border-gray-200">
      <div class="flex items-center space-x-3">
        <div class="w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center">
          <span class="text-white text-sm font-medium">{{ userType === 'accountant' ? 'C' : 'A' }}</span>
        </div>
...
<script setup>
import { HomeIcon, CreditCardIcon, ChartBarIcon, ChatBubbleLeftRightIcon } from '@heroicons/vue/24/outline';
...
defineProps({
  ...
  userType: {
    type: String,
    default: 'accountant' // 'accountant' or 'client'
  }
});
